...
try{
	...
	if(!empty($_POST['conclusao']))
	{
		...
		$IDquest = $_POST['questionarios'][0];
			
		$conexao6 = conn_mysql();
	        
		$SQLBusca = 'SELECT * FROM funcionarios';
			
		$SQLUpdate = 'UPDATE avaliacao SET conclusao=?, IDquestionario=? WHERE IDavaliacao=?';
		...
		$atualizacao = $operacao6->execute(array($conclusao, $IDquest, $IDavaliacao));
	...
	$conexao = conn_mysql();
		...
		echo '<div class="panel-default panel">';
			echo '<div class="panel-heading">';
				echo '<h2 class="sub-header">Escolha qual questionário para a avaliação</h2>';
				echo '</div>';
				...
				foreach($resultados5 as $questionariosEncontrados)
						{
							echo '<div class="row-fluid panel-body">';
							echo '<input type="radio" name="questionarios[]" value="'.$questionariosEncontrados['IDquestionario'].'" ';
							
							if($resultados[0]['IDquestionario'] == $questionariosEncontrados['IDquestionario'])
							{
								echo 'checked';
							}
		...
		echo '<div class="panel-default panel">';
			echo '<div class="panel-heading">';	
				echo '<h2 class="sub-header">Insira sua Conclusão</h2>';
				echo '<div class="row-fluid panel-body">';
					foreach($resultados as $avaliacoesEncontradas){
