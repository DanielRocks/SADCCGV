...
session_start();
	...
	require_once("./conf/confBD.php");
      ...
      elseif(isset($_COOKIE["loginAutomatico"])){ 	//existe um cookie com nome senha --> login automático
            $log = utf8_encode(htmlspecialchars($_COOKIE["loginSAD"]));
 ...
 try{
		...
		$SQLSelect = 'SELECT * FROM funcionarios WHERE senha=MD5(?) AND login=?';
		...
		if (count($resultados)!=1){	
			...
			setcookie("loginSAD", $log, time()+60*60*24*90); //guarda o login por 90 dias a partir de agora
			if(!empty($lembrar)){
		   ...
		   $_SESSION['ID'] = $resultados[0]['IDfuncionario'];
		   $_SESSION['login'] = $log;
